 ...
 */
const switchOutputDevice = (devices, currentDevice, flag = true) => {
	const index = devices.findIndex((value) => value === currentDevice);
	const diff = flag ? 1 : -1;
	const nextIndex = (index + diff + devices.length) % devices.length;

	const temp = execSync(`pacmd set-default-sink ${devices[nextIndex]}`, {encoding: 'utf-8'});
